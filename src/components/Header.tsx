...
const Header = ({ title, img }: HeaderProps) => {
  ...
  return (
    <>
      <div className="w-full max-w-[470px] flex items-center justify-between py-[30px] px-[25px]">
        <button onClick={() => navigate(-1)} className="">
          <Allow className="w-6 h-5" />
        </button>
        {title && (
          <span className="text-center font-normal text-black text-[20px] leading-[120%]">
            {title}
          </span>
        )}
